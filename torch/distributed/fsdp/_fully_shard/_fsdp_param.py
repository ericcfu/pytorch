...
class FSDPParam:
    ...
    def _init_sharded_param(
        ...
        if not self.is_dtensor:
            # Non-TP case: use MemoryShardedDTensor for memory-efficient storage sharding
            self.sharded_param = nn.Parameter(
                self._to_memory_sharded_dtensor(
                    sharded_param, self._orig_size, shard_dim, padded_local=padded_local
                )
            )
        else:
            # TP case: use MemoryShardedDTensor with combined SPMD mesh and placements
            self.sharded_param = nn.Parameter(
                self._to_memory_sharded_dtensor(
                    sharded_param,
                    self._orig_size,
    ...
    def to_memory_sharded_dtensor_grad(
        ...
        """
        if self.is_dtensor:
            # TP case: use combined SPMD mesh and placements
